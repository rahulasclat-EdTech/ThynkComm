...
const THEMES = {
  "Ocean Teal": {
    bg: "#0f1923", sidebar: "#131f2e", card: "#1a2940", hover: "#1f3350",
    accent: "#1ab8a8", accent2: "#0e8a7d", accentLight: "#1ab8a820",
    border: "#ffffff14", text: "#f0f8ff", sub: "#7fa8c4",
    red: "#ff5c7a", yellow: "#ffd166", blue: "#4db8ff", purple: "#c77dff", green: "#06d6a0", orange: "#ff9f43",
    gradMain: "linear-gradient(135deg, #1ab8a8, #0e6e85)",
  },
  "Midnight Purple": {
    bg: "#0e0b1a", sidebar: "#140f24", card: "#1e1535", hover: "#2a1d50",
    accent: "#a855f7", accent2: "#7c3aed", accentLight: "#a855f720",
    border: "#ffffff14", text: "#f3f0ff", sub: "#9d8fbf",
    red: "#ff5c7a", yellow: "#ffd166", blue: "#60a5fa", purple: "#c77dff", green: "#34d399", orange: "#fb923c",
    gradMain: "linear-gradient(135deg, #a855f7, #7c3aed)",
  },
  "Sunset Orange": {
    bg: "#1a0f0a", sidebar: "#231208", card: "#2d1a10", hover: "#3d2415",
    accent: "#f97316", accent2: "#ea580c", accentLight: "#f9731620",
    border: "#ffffff14", text: "#fff7ed", sub: "#c4956a",
    red: "#ef4444", yellow: "#fbbf24", blue: "#38bdf8", purple: "#c084fc", green: "#4ade80", orange: "#fb923c",
    gradMain: "linear-gradient(135deg, #f97316, #ea580c)",
  },
  "Forest Green": {
    bg: "#0a1a0f", sidebar: "#0d2114", card: "#132b1a", hover: "#1a3a23",
    accent: "#22c55e", accent2: "#16a34a", accentLight: "#22c55e20",
    border: "#ffffff14", text: "#f0fff4", sub: "#6aab7e",
    red: "#f87171", yellow: "#facc15", blue: "#38bdf8", purple: "#c084fc", green: "#4ade80", orange: "#fb923c",
    gradMain: "linear-gradient(135deg, #22c55e, #16a34a)",
  },
  "Rose Pink": {
    bg: "#1a0a12", sidebar: "#230e18", card: "#2d1220", hover: "#3d1a2d",
    accent: "#f43f5e", accent2: "#e11d48", accentLight: "#f43f5e20",
    border: "#ffffff14", text: "#fff1f2", sub: "#c47a8e",
    red: "#ff5c7a", yellow: "#fbbf24", blue: "#38bdf8", purple: "#c084fc", green: "#34d399", orange: "#fb923c",
    gradMain: "linear-gradient(135deg, #f43f5e, #e11d48)",
  },
  "Steel Blue": {
    bg: "#0a1220", sidebar: "#0e1828", card: "#132030", hover: "#1a2d42",
    accent: "#3b82f6", accent2: "#2563eb", accentLight: "#3b82f620",
    border: "#ffffff14", text: "#eff6ff", sub: "#7ea8cc",
    red: "#f87171", yellow: "#fbbf24", blue: "#60a5fa", purple: "#a78bfa", green: "#34d399", orange: "#fb923c",
    gradMain: "linear-gradient(135deg, #3b82f6, #2563eb)",
  },
  "Golden Dark": {
    bg: "#141008", sidebar: "#1c160a", card: "#251d0e", hover: "#332810",
    accent: "#f59e0b", accent2: "#d97706", accentLight: "#f59e0b20",
    border: "#ffffff14", text: "#fffbeb", sub: "#b59a5a",
    red: "#f87171", yellow: "#fcd34d", blue: "#38bdf8", purple: "#c084fc", green: "#4ade80", orange: "#fb923c",
    gradMain: "linear-gradient(135deg, #f59e0b, #d97706)",
  },
  "Arctic White": {
    bg: "#f0f4f8", sidebar: "#e2e8f0", card: "#ffffff", hover: "#dde5ef",
    accent: "#0ea5e9", accent2: "#0284c7", accentLight: "#0ea5e915",
    border: "#00000012", text: "#0f172a", sub: "#64748b",
    red: "#ef4444", yellow: "#f59e0b", blue: "#3b82f6", purple: "#8b5cf6", green: "#10b981", orange: "#f97316",
    gradMain: "linear-gradient(135deg, #0ea5e9, #0284c7)",
  },
};

// Active theme — loads from localStorage or defaults to Ocean Teal
const _savedTheme = (() => { try { return localStorage.getItem("thynk_theme") || "Ocean Teal"; } catch { return "Ocean Teal"; } })();
let _activeThemeName = THEMES[_savedTheme] ? _savedTheme : "Ocean Teal";

// C is a proxy so all existing C.xxx references update when theme changes
const C = new Proxy({}, {
  get(_, key) {
    const base = THEMES[_activeThemeName];
    if (key in base) return base[key];
    // fallback gradients
    if (key === "gradPurple")  return "linear-gradient(135deg, #c77dff, #7b2fff)";
    if (key === "gradBlue")    return "linear-gradient(135deg, #4db8ff, #1a6aff)";
    if (key === "gradOrange")  return "linear-gradient(135deg, #ff9f43, #ff6b35)";
    if (key === "gradRed")     return "linear-gradient(135deg, #ff5c7a, #c0392b)";
    if (key === "gradGreen")   return "linear-gradient(135deg, #06d6a0, #0e8a7d)";
    if (key === "gradMain")    return base.gradMain;
    return "#888";
  }
});
...
function applyTheme(name) {
  ...
  try { localStorage.setItem("thynk_theme", name); } catch {}
  // Force re-render by dispatching a custom event
  window.dispatchEvent(new Event("thynk_theme_change"));
}
...
function Contacts() {
  ...
  const saveEdit = async () => {
    if (!editForm.name || !editForm.phone) return alert("Name and phone required");
    setSaving(true);
    await fetch(`/api/contacts?id=${editContact.id}`, {
      method: "PUT",
    ...
    <div>
      ...
      {editContact && (
        <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.7)", zIndex:9999, display:"flex", alignItems:"center", justifyContent:"center", padding:16 }}>
          <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:14, width:"100%", maxWidth:480, padding:24 }}>
            <h3 style={{ margin:"0 0 16px", fontSize:15, fontWeight:800 }}>✏️ Edit Contact</h3>
            <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
              <div><label style={{ fontSize:12, color:C.sub, fontWeight:700 }}>NAME *</label><input value={editForm.name||""} onChange={e=>setEditForm({...editForm,name:e.target.value})} style={{ ...inp, marginTop:4 }} /></div>
              <div><label style={{ fontSize:12, color:C.sub, fontWeight:700 }}>PHONE *</label><input value={editForm.phone||""} onChange={e=>setEditForm({...editForm,phone:e.target.value})} style={{ ...inp, marginTop:4 }} /></div>
              <div><label style={{ fontSize:12, color:C.sub, fontWeight:700 }}>EMAIL</label><input value={editForm.email||""} onChange={e=>setEditForm({...editForm,email:e.target.value})} style={{ ...inp, marginTop:4 }} /></div>
              <div><label style={{ fontSize:12, color:C.sub, fontWeight:700 }}>TAG</label>
                <select value={editForm.tag||"Lead"} onChange={e=>setEditForm({...editForm,tag:e.target.value})} style={{ ...inp, marginTop:4 }}>
                  <option>Lead</option><option>Customer</option><option>VIP</option>
                </select>
              </div>
            </div>
            <div style={{ display:"flex", gap:10, marginTop:16 }}>
              <button style={btn()} onClick={saveEdit} disabled={saving}>{saving?"Saving...":"💾 Save"}</button>
              <button style={btn("secondary")} onClick={()=>setEditContact(null)}>Cancel</button>
            </div>
          </div>
        </div>
      )}
      ...
      <div style={card}>
        ...
        {loading ? <Loader /> : filteredContacts.length === 0 ? (
          ...
          <table style={{ width:"100%", borderCollapse:"collapse" }}>
            <thead><tr>{["Name","Phone","Email","Tag","Opt-In","Actions"].map(h=><th key={h} style={{ textAlign:"left", padding:"10px 12px", fontSize:11, color:C.sub, fontWeight:700, borderBottom:`1px solid ${C.border}` }}>{h}</th>)}</tr></thead>
            <tbody>
                ...
                <tr key={c.id} style={{ borderBottom:`1px solid ${C.border}` }}>
                  ...
                  <td style={{ padding:"12px" }}>
                    <div style={{ display:"flex", gap:6 }}>
                      <button onClick={()=>{ setEditContact(c); setEditForm({name:c.name,phone:c.phone,email:c.email||"",tag:c.tag||"Lead",opt_in:c.opt_in}); }}
                        style={{ ...btn("secondary"), fontSize:11, padding:"4px 10px" }}>✏️ Edit</button>
                      <button onClick={()=>deleteContact(c.id)}
                        style={{ ...btn("secondary"), fontSize:11, padding:"4px 10px", color:C.red }}>🗑️</button>
                    </div>
                  </td>
                </tr>
              ))}
...
function CreateCampaign() {
  ...
  const [templateParams, setTemplateParams] = useState([]);  // values for {{1}}, {{2}}... variables
  const [status, setStatus]         = useState(null);
      ...
      if (scheduleType === "scheduled") {
        ...
        const r = await fetch("/api/campaigns/schedule", {
          ...
          body: JSON.stringify({
            ...
            template_params: msgType==="template" && templateParams.filter(Boolean).length > 0 ? templateParams.filter(Boolean) : undefined,
  ...
  return (
    <div>
      ...
      {step===2 && (<div style={card}><h3 style={{ margin:"0 0 14px", fontSize:15, fontWeight:700 }}>Message Type</h3><div style={{ display:"flex", gap:10, marginBottom:18 }}>{[["text","✏️ Custom Text"],["template","📋 Meta Template"]].map(([val,label])=>(<button key={val} onClick={()=>setMsgType(val)} style={{ ...btn(msgType===val?"primary":"secondary"), fontSize:13 }}>{label}</button>))}</div>{msgType==="text"&&(<div><label style={{ fontSize:12, color:C.sub, fontWeight:700 }}>MESSAGE TEXT *</label><textarea value={message} onChange={e=>setMessage(e.target.value)} style={{ ...inp, minHeight:140, resize:"vertical", marginTop:6 }} placeholder="Type your message..." /><div style={{ fontSize:11, color:C.sub, marginTop:5 }}>{message.length}/4096</div></div>)}{msgType==="template"&&(<div style={{ display:"flex", flexDirection:"column", gap:12 }}><div style={{ padding:"12px 16px", background:"#fffbeb", border:"1px solid #fde68a", borderRadius:10, fontSize:12, color:"#92400e" }}>ℹ️ Meta-approved templates send immediately. Locally created templates also appear — ensure they are approved in Meta Business Manager for live sends.</div><div><label style={{ fontSize:12, color:C.sub, fontWeight:700 }}>SELECT TEMPLATE *</label>{tplLoading ? (<div style={{ fontSize:12, color:C.sub, marginTop:8, padding:"10px 14px", background:C.bg, borderRadius:10 }}>⏳ Loading templates from Meta...</div>) : (<select value={templateName} onChange={e=>{ const t=templates.find(x=>x.name===e.target.value); setTemplateName(e.target.value); setTemplateParams([]); if(t) setTemplateLang(t.language||"en_US"); }} style={{ ...inp, marginTop:6 }}><option value="">— Select a template —</option>{templates.filter(t=>!t.isLocal).length>0&&(<optgroup label="✅ Meta Approved">{templates.filter(t=>!t.isLocal).map(t=>(<option key={`${t.name}_${t.language}`} value={t.name}>{t.name} ({t.language}) — {t.category}</option>))}</optgroup>)}{templates.filter(t=>t.isLocal).length>0&&(<optgroup label="📝 Locally Created">{templates.filter(t=>t.isLocal).map(t=>(<option key={t.name} value={t.name}>{t.name}</option>))}</optgroup>)}</select>)}{templateName && (() => {
                  const tpl = templates.find(t=>t.name===templateName);
                  const preview = tpl?.preview || "";
                  const varMatches = preview.match(/\{\{\d+\}\}/g) || [];
                  const vars = [...new Set(varMatches)].sort();
                  return (<>
                    {preview && <div style={{ marginTop:8, padding:"10px 14px", background:C.accentLight, borderRadius:10, fontSize:12, color:C.accent2, border:`1px solid ${C.accent}30` }}><strong>Preview:</strong> {preview}</div>}
                    {vars.length > 0 && (
                      <div style={{ marginTop:10, padding:"12px 14px", background:C.bg, borderRadius:10, border:`1px solid ${C.border}` }}>
                        <div style={{ fontSize:12, color:C.yellow, fontWeight:700, marginBottom:10 }}>📝 This template has {vars.length} variable(s) — fill them in:</div>
                        {vars.map((v, i) => (
                          <div key={v} style={{ marginBottom:8 }}>
                            <label style={{ fontSize:12, color:C.sub, fontWeight:700 }}>{v} *</label>
                            <input value={templateParams[i]||""} onChange={e=>{ const p=[...templateParams]; p[i]=e.target.value; setTemplateParams(p); }} style={{ ...inp, marginTop:4 }} placeholder={`Value for ${v} (same for all recipients)`} />
                          </div>
                        ))}
                        <div style={{ fontSize:11, color:C.sub, marginTop:4 }}>⚠️ Template will FAIL if variables are left empty</div>
                      </div>
                    )}
                  </>);
...
function MessageLog() {
  ...
  useEffect(() => {
    fetchLogs();
    // Auto-refresh every 30 seconds to pick up delivered/read status updates from webhook
  ...
  const exportCSV = () => {
    const header = "time,to_number,message,template_name,status,failure_reason,direction,source,tag";
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(7,1fr)", gap:8, marginBottom:18 }}>
        {[
          ...
          <div key={label} style={{ ...card, padding:"10px 12px", textAlign:"center",
                                    background:bg, border:`1px solid ${color}30`, cursor:"pointer" }}
               onClick={()=>{
                 if(label==="Portal") { setSourceFilter("portal"); setPage(1); }
                 else if(label==="API") { setSourceFilter("api"); setPage(1); }
                 else if(label==="Sent") { setStatusFilter("sent"); setPage(1); }
                 else if(label==="Delivered") { setStatusFilter("delivered"); setPage(1); }
                 else if(label==="Read") { setStatusFilter("read"); setPage(1); }
                 else if(label==="Failed") { setStatusFilter("failed"); setPage(1); }
                 else { setStatusFilter("all"); setSourceFilter("all"); setPage(1); }
               }}>
            ...
            <table style={{ width:"100%", borderCollapse:"collapse", fontSize:13 }}>
              <thead>
                <tr style={{ borderBottom:`1px solid ${C.border}`, background:"#0d1821" }}>
                  {["Time","Recipient","Message / Template","Status","Failure Reason","Dir","Source"].map(h => (
                  ...
                  return (
                    <tr key={m.id||i}
                      ...
                      <td style={{ padding:"10px 14px" }}>
                        <span style={{ ...pill(sc,sbg), fontSize:11, padding:"3px 9px", display:"inline-flex", alignItems:"center", gap:4 }}>
                          { m.status==="delivered" ? "📨" : m.status==="read" ? "👁️" : m.status==="sent" ? "✅" : m.status==="failed" ? "❌" : "⏳" }
                          {" "}{(m.status||"unknown")}
                        </span>
                      </td>
                      <td style={{ padding:"10px 14px", maxWidth:220, fontSize:11 }}>
                        {m.status==="failed" && m.error_detail ? (
                          <span style={{ color:C.red, display:"block", wordBreak:"break-word", lineHeight:1.4 }}
                            title={m.error_detail}>
                            {m.error_detail.length > 60 ? m.error_detail.slice(0,60)+"…" : m.error_detail}
                          </span>
                        ) : m.status==="failed" && m.error_code ? (
                          <span style={{ color:C.red, fontSize:11 }}>Error code: {m.error_code}</span>
                        ) : (
                          <span style={{ color:C.sub }}>—</span>
                        )}
                      </td>
...
function ThemeSwitcher() {
  const [current, setCurrent] = useState(_activeThemeName);
  const [open, setOpen]       = useState(false);

  useEffect(() => {
    const handler = () => setCurrent(_activeThemeName);
    window.addEventListener("thynk_theme_change", handler);
    return () => window.removeEventListener("thynk_theme_change", handler);
  }, []);

  const THEME_PREVIEWS = {
    "Ocean Teal":       ["#0f1923", "#1ab8a8"],
    "Midnight Purple":  ["#0e0b1a", "#a855f7"],
    "Sunset Orange":    ["#1a0f0a", "#f97316"],
    "Forest Green":     ["#0a1a0f", "#22c55e"],
    "Rose Pink":        ["#1a0a12", "#f43f5e"],
    "Steel Blue":       ["#0a1220", "#3b82f6"],
    "Golden Dark":      ["#141008", "#f59e0b"],
    "Arctic White":     ["#f0f4f8", "#0ea5e9"],
  };
  ...
  return (
    <div style={{ position:"relative" }}>
      <button onClick={()=>setOpen(!open)}
        title="Change Theme"
        style={{ background:"transparent", border:`1px solid ${C.border}`, borderRadius:8, padding:"6px 10px", cursor:"pointer", display:"flex", alignItems:"center", gap:6, color:C.sub, fontSize:12 }}>
        <div style={{ display:"flex", gap:3 }}>
          {["#1ab8a8","#a855f7","#f97316","#3b82f6"].map(c=>(
      ...
      {open && (
        <div style={{ position:"absolute", top:"calc(100% + 8px)", right:0, background:C.card, border:`1px solid ${C.border}`, borderRadius:12, padding:10, zIndex:9999, width:220, boxShadow:"0 8px 32px #00000040" }}>
          <div style={{ fontSize:11, color:C.sub, fontWeight:700, marginBottom:8, paddingLeft:4 }}>CHOOSE THEME</div>
          {Object.entries(THEME_PREVIEWS).map(([name, [bg, accent]])=>(
            <button key={name} onClick={()=>{ applyTheme(name); setCurrent(name); setOpen(false); }}
              style={{ display:"flex", alignItems:"center", gap:10, width:"100%", padding:"8px 10px", borderRadius:8, border:`1px solid ${current===name ? accent : "transparent"}`, background: current===name ? accent+"15" : "transparent", cursor:"pointer", marginBottom:2 }}>
              {/* Mini preview */}
              <div style={{ width:32, height:22, borderRadius:5, background:bg, border:`1px solid ${accent}40`, display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0 }}>
                <div style={{ width:12, height:4, borderRadius:2, background:accent }} />
              </div>
              <span style={{ fontSize:13, color: current===name ? accent : C.text, fontWeight: current===name ? 700 : 400 }}>{name}</span>
              {current===name && <span style={{ marginLeft:"auto", color:accent, fontSize:14 }}>✓</span>}
            </button>
          ))}
        </div>
      )}
  ...
  return (
    <div style={{ display:"flex", height:"100vh", background:C.bg, fontFamily:"'Plus Jakarta Sans','DM Sans','Segoe UI',sans-serif", color:C.text, overflow:"hidden" }}>
      ...
      <div style={{ flex:1, display:"flex", flexDirection:"column", overflow:"hidden" }}>
        <div style={{ padding:"14px 24px", borderBottom:`1px solid ${C.border}`, display:"flex", alignItems:"center", justifyContent:"space-between", background:`${C.sidebar}ee`, backdropFilter:"blur(12px)", flexShrink:0, position:"relative", zIndex:100 }}>
          <div>
